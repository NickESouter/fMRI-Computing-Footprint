...
#(b) the location where FSF files should be saved, and (c) the location of FSF templates.
BIDSdir = '<full path redacted>/CNP_BIDS/'
fsfdir = '<full path redacted>/FSL/fsf_files/'
templates = '<full path redacted>/FSL/fsf_templates/'
...
for subject in sorted(os.listdir(BIDSdir)):

	#If a subfolder doesn't correspond to a specific subject, it's skipped.
	if 'sub' not in subject:
		continue

	#Defines the location of the preprocessing fsf template, and the preprocessing output file.
	...
	with open(preproc_template) as infile:
		with open(preproc_output, 'w') as outfile:
			...
			#Iterates over each line, and replaces any instance of subject placeholders with subject ID.
			for line in infile:
	...
	with open(stats_template) as infile:
		with open(stats_output, 'w') as outfile:
			...
			#Iterates over each line in the template, and replaces ID placeholders.
			for line in infile:
